...
module internal Targets =
        ...
        let init safe =
            ...
            Target.create "WatchTests" (fun _ ->
                ...
                [
                    ...
                    "client",
                    dotnet [ "fable"; "watch"; "-o"; "output"; "-s"; "--run"; "npx"; "vite" ] safe.ClientTestsPath
                ]
                |> runParallel)
            ...
            [
                ...
                "SafeClean" ==> "AssemblyInfo" ==> "InstallClient" ==> "Build"
    ...
    let init (definition: ProjectDefinition) =
        ...
        Target.create "Info" (fun _ ->
            let separator sign =
                Trace.traceFAKE "%s" (String.replicate 69 sign)
            ...
            Trace.tracefn
                "Git.branch: %s"
                (definition.Project.Git
                 |> Option.map (fun git -> git.Branch)
                 |> Option.defaultValue "unknown")

            Trace.tracefn
                "Git.commit: %s"
                (definition.Project.Git
                 |> Option.map (fun git -> git.Commit)
                 |> Option.defaultValue "unknown")

            Trace.tracefn
                "Git.repository: %s"
                (definition.Project.Git
                 |> Option.bind (fun git -> git.Repository)
                 |> Option.defaultValue "unknown")
            ...
            let getAssemblyInfoAttributes projectName =
                ...
                let release =
                    ...
                    |> Option.bind (fun changeLog ->
                        try
                            ReleaseNotes.parse (
                                System.IO.File.ReadAllLines changeLog |> Seq.filter ((<>) "## Unreleased")
                            )
                            |> Some
                        with _ ->
                            None)
        ...
        if not definition.Specs.IsSAFEStack then
            ...
            Target.create "Release" (fun _ ->
                ...
                | {
                    ...
                    | releaseSource when releaseSource |> Directory.Exists -> Dotnet.runOrFail "pack" releaseSource
                    | _ -> Dotnet.runInRootOrFail "pack"

                    Directory.ensure releaseDir

                    !!"**/bin/**/*.nupkg" |> Seq.iter (Shell.moveFile releaseDir)

                | {
                      Specs = ConsoleApplication {
                    ...
                    !!(releaseDir </> "*.zip") ++ (releaseDir </> "*.tar.gz")
                    ...
                    seq {
                        ...
                        yield!
                            runtimeIds
                            |> List.collect (RuntimeId.value >> fun runtimeId -> [ project, runtimeId ])
                    }
                    |> Seq.iter (fun (project, runtimeId) ->
                        sprintf
                            "publish -c Release /p:PublishSingleFile=true -o %s/%s --self-contained -r %s %s"
                            releaseDir
                            runtimeId
                            runtimeId
                            project
                ...
                | { Specs = Executable { ReleaseDir = releaseDir } } ->
                    releaseDir |> sprintf "publish -c Release -o %s" |> Dotnet.runInRootOrFail

                | { Specs = SAFEStackApplication _ } ->
                    failwithf "For releasing SAFE-Stack Application, use \"bundle\" target instead.")
            ...
            Target.create "Publish" (fun _ ->
                ...
                | {
                      Specs = Library {
                                          ReleaseDir = releaseDir
                                          NugetApi = NugetApi.Organization organization
                                          NugetCustomServerRepository = nugetServer
                                      }
                  } ->
                    ...
                    match
                        envVar "NUGET_SERVER_TOKEN", envVar "NUGET_SERVER_REPOSITORY" |> Option.orElse nugetServer
                    with
                ...
                | {
                      Specs = Library {
                                          ReleaseDir = releaseDir
                                          NugetApi = NugetApi.KeyInEnvironment name
                                          Organization = organization
                                      }
                  } ->
                    Trace.traceHeader "Pushing to nuget server"

                    envVar name |> Option.iter (Nuget.push releaseDir organization)
