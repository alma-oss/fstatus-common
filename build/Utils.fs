...
module internal Utils =
    ...
    let envVar name =
        if Environment.hasEnvironVar (name) then
            Environment.environVar (name) |> Some
        else
            None
        ...
        let push releaseDir organization token =
            let sourceName =
                ...
                |> Option.map (fun organization ->
                    ...
                    sprintf
                        "nuget add source --username %s --password %s --store-password-in-clear-text --name %s \"https://nuget.pkg.github.com/%s/index.json\""
                        organization
                        token
                        sourceName
                        organization
    ...
    module ProjectDefinition =
        ...
        and ProjectMeta = { Name: string; Summary: string; Git: Git option }
        ...
        module Spec =
            let defaultLibrary: ProjectSpec =
                let sources = !!"./*.fsproj" ++ "src/*.fsproj" ++ "src/**/*.fsproj"
                ...
                Library {
                    ...
                    AllSources = sources ++ "tests/*.fsproj" ++ "build/*.fsproj"
                    Organization = None
            ...
            let defaultExecutable: ProjectSpec =
                let release = !!"./*.fsproj" ++ "src/**/*.fsproj"

                Executable {
                    Changelog =
                        if File.Exists "CHANGELOG.md" then
                            Some "CHANGELOG.md"
                        else
                            None
                    ReleaseDir = "/app"
                    ...
                    TestsSources = !!"tests/**/*.fsproj"
                    AllSources = release ++ "tests/**/*.fsproj" ++ "build/*.fsproj"
                }
            ...
            let mapConsoleApplication f =
                function
                | ConsoleApplication spec -> f spec |> ConsoleApplication
                | spec -> spec

            let mapSAFEStackApplication f =
                function
    ...
    module Http =
        ...
        let post (currentProject: string) token (url: string) (data: string) =
            async {
    ...
    module Github =
        ...
        let triggerAction
            ...
            async {
                ...
                let data =
                    sprintf @"{""event_type"":""%s"",""client_payload"":{""from"": ""%s""}}" event current
